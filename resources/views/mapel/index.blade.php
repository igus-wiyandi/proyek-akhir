    ...
    <div class="w-full max-w-7xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        @endif
         ...
         <div class="bg-teal-600 p-4 flex justify-between items-center">
            <h2 class="text-xl font-semibold text-white">Kelas 10</h2>
            <a href="{{ route('mapel.create') }}" class="text-white hover:text-teal-100 transition-colors" title="Tambah Data">
                         ...
                         <tr class="hover:bg-gray-50">
                           ...
                           <td class="p-3 border border-gray-200">
                            <div class="flex space-x-3">
                                <a href="{{ route('mapel.edit', $m->id) }}" class="text-teal-600 hover:text-teal-800 transition-colors" title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                                </a>
                                <form action="{{ route('mapel.destroy', $m->id) }}" method="POST" onsubmit="return confirm('Yakin ingin dihapus?')">
                                    @csrf
                                    @method('DELETE')
...
<div id="guruModal" class="fixed inset-0 bg-black bg-opacity-50 hidden justify-center items-center z-50">
    <div class="bg-white w-full max-w-md p-6 rounded shadow-lg relative">
        ...
        <form id="guruForm" method="POST" action="">
            @csrf
...
<script>
    function openGuruModal(mapelId, mapelNama, guruIdTerpilih = null) {
        ...
        form.action = `/mapel/${mapelId}/pilih-guru`;
