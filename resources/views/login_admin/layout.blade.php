  ...
  <main class="w-9/12 max-h-screen rounded-2xl flex shadow-2xl">
    ...
    <form action="{{ route('prosesloginAdmin') }}" method="POST"
      ...
      <div class="flex flex-col w-full mt-10 relative">
        ...
        <img src="{{ asset('images/email.png.png') }}" alt="email" class="absolute left-3 top-8 flex items-center w-6 h-6">
      ...
      <div class="flex text-sm text-slate-600 font-semibold mt-5">
        ...
        <a href="{{ route('loginGuru') }}" class="hover:underline text-teal-500">Kembali ke Guru!</a>
      </div>
    </form>

    <section class="w-full h-full bg-teal-500 rounded-r-2xl px-5 py-8 pb-12 flex flex-col items-center">
        ...
        <div class="flex flex-col w-full items-center text-white font-semibold text-sm mt-5">
          <p>Masukkan kredensial Anda untuk mengakses halaman admin</p>
          <p>dan mulai mengecek data absen dan gaji</p>
        </div>

        <img src="{{ asset('images/admin.png.png') }}" alt="" class="h-96 mt-10">
      </section>
  </main>
